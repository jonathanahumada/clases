...
\date{<2019-02-9 Sun>}
\title{Introducción a Sistemas Operativos Sistema operativo: Definición Procesos Procesos$_{\text{2}}$}
\hypersetup{
  pdfkeywords={},
  pdfsubject={},
...
\label{sec-1-2}
\begin{itemize}
\item Descripción y control de los procesos
...
\item Planificación en sistemas operativos
\item Gestión de entrada/salida y planificacion de disco
\item Gestion de archivos
\item Introduccion a la Seguridad en S.O
\item Introducción a las redes
\end{itemize}
\section{Notas}
\label{sec-3}
\begin{itemize}
...
\subsection{Cómo se definiría un sistema operativo?}
\label{sec-3-1}
...
\item Detección y respuesta a errores
\item Estadísticias de utilización de recursos
\end{enumerate}
\end{enumerate}
...
\begin{enumerate}
\item Kernel : programa más importante
\label{sec-3-1-1-2-1}
\end{enumerate}
\end{enumerate}
...
\label{sec-3-1-2}
\begin{enumerate}
\item Descripción y control de procesos
\label{sec-3-1-2-1}
...
\item Paginación (bloques de) = recursos memoria a diferentes aplicaciones.
\end{enumerate}
\item Planificación
\label{sec-3-1-2-4}
...
\item Gestión de entrada y salida
\label{sec-3-1-2-5}
Puertos
\item Gestion de archivos
\label{sec-3-1-2-6}
...
\begin{itemize}
\item Firewall
\item Privilegios de usuario / lectura /escritura
\end{itemize}
\item Gestion de redes
\label{sec-3-1-2-8}

\begin{itemize}
\item Todo OS moderno debe tener esto.
\item la ´nube'' = red de servidores de topología indefinida.
\end{itemize}
\end{enumerate}

\subsubsection{Llamadas al sistema}
\label{sec-3-1-3}
\subsubsection{Evolución del OS}
\label{sec-3-1-4}
\begin{enumerate}
\item Hombre-máquina = no había OS. Solo procesos en serie.
\item Procesos por lotes = procesos en serie, monitor del sistema  (cargabas los programas y luego se ejecutaban en serie)
\item Proceos por lotes con multiprogramación
...
\item contexto de ejecución
\end{enumerate}
\item ¿Qué datos están asociados al proceso?
\label{sec-3-1-5-1-2}
\begin{quote}
...
-- Jesus Carretero
\end{quote}
\item ¿Qué es un entorno de ejecución?
\label{sec-3-1-5-1-3}
\begin{quote}
...
\begin{enumerate}
\item sincronización incorrecta
\item fallos de exclusión mutua:
\end{enumerate}
...
\item Herramientas de supervisión de recursos
\label{sec-3-1-5-2}
\begin{enumerate}
\item tipos
\label{sec-3-1-5-2-1}
...
\begin{enumerate}
\item Estadística. Ej: servidores.
\end{enumerate}
\end{enumerate}
\item Qué se supervisa
\label{sec-3-1-5-2-2}
\begin{enumerate}
\item Memoria
\item Disco
...
\item un proceso puede pasar de liso a terminado sin pasar por ejecución.
\end{itemize}
\item colas o hilos
